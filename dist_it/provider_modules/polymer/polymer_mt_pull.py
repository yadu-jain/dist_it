...
def start_pull(process_id,response=None):
	...
	res_from_cities=polymer_mt_api.get_from_cities(process_id=process_id)
	print "start_pulling_to_cities."
	polymer_mt_api.get_to_cities(process_id=process_id,from_city_response=res_from_cities)
	print "Get city pairs to pull routes."
	response_city_pairs=polymer_mt_api.get_city_pairs(process_id=process_id)
	...
	response_route_codes=polymer_mt_api.get_route_codes(process_id=process_id)
...
def start_pulling_pickups(process_id,response):
	...
	for route_code in response:
		job=("polymer.polymer_mt_api","get_pickups",{"process_id":process_id,"route_code":route_code["route_code"]})
		# manager.add_job(job,
...
def start_pulling_dropoffs(process_id,response):
	...
	waiter=jobs.JobsWaiter(manager,process_id)
	for route_code in response:
		job=("polymer.polymer_mt_api","get_dropoffs",{"process_id":process_id,"route_code":route_code["route_code"]})
		# manager.add_job(job,
			# callback_list=[waiter.get_callback_job()])				
		waiter.add_job(job)
...
def process_data(process_id,response=None):
	...
	""" 
	polymer_mt_api.process_data(process_id=process_id)
